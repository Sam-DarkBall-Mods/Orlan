...
class CfgVehicles
{
	...
	class lancet_tripod_launcher: StaticWeapon
	{
		model="\orlan\tripod\lancet_tripod.p3d";
		crew = "O_soldier_f";
		...
		class UserActions
		{
			class ReloadAction
			{
				...
				showWindow=0;
				onlyForPlayer=1;
				condition="(vehicle player == player) && (isnull (this getvariable ['proxy',objnull])) && !(someAmmo this) || ('lancet_dummy_mag' in magazines player)";
	...
	class Lancet_Tripod_Bag_Weapon: Weapon_Bag_Base
	{
		...
		class assembleInfo
		{
			assembleTo="lancet_tripod_launcher";
			base[]=
			{
				"Lancet_Tripod_Bag_Support"
			};
